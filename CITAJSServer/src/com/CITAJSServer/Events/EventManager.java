...
public class EventManager 
{
	...
	ActionHandler_Client actionHandler_Client;
	...
	{		
		...
		actionHandler_Client = new ActionHandler_Client(taskContext);
	...
	class EventHandler extends Thread
	{
		...
		{	
			...
			else
			{
				if (event.getEventType() == EventType.CALLBACK_DATA_EVENT)
				{
					actionHandler_Client.handle(file, func, event.encode());
				}
				if (event.getEventType() == EventType.CALL_PARAM_EVENT)
				{
					actionHandler_Client.handle(engine, file, func, event.encode());
				}
				else if (event.getEventType() == EventType.CALL_NOPARAM_EVENT)
				{
					actionHandler_Client.handle(engine, file, func);
				}
			}
		}
	}
