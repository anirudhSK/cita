...
shell.done = function(req) {
  if (req.readyState == this.DONE_STATE && req.status==200) {
    var statement = document.getElementById('statement')
    ...
    document.getElementById('ajax-status').innerHTML = "Done!";
  }
};
...
shell.runStatement = function() {
  ...
  document.getElementById('ajax-status').innerHTML = "processing";
